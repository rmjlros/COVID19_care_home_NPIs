...
CQC_care_homes <- CQC_care_homes %>%
  filter(`Care home?` == "Y") %>%
  filter(`Care homes beds` != 0)
CQC_care_homes_older_people <- CQC_care_homes %>% filter(`Service user band - Older People` == "Y")
CQC_care_homes_older_people_nh <- CQC_care_homes_older_people %>%
  filter(`Service type - Care home service with nursing` == "Y") %>%
  filter(is.na(`Service type - Care home service without nursing`))
CQC_care_homes_older_people_res <- CQC_care_homes_older_people %>%
  filter(is.na(`Service type - Care home service with nursing`)) %>%
  filter(`Service type - Care home service without nursing` == "Y")
CQC_care_homes_older_people_nh_res <- CQC_care_homes_older_people %>%
  filter(`Service type - Care home service with nursing` == "Y") %>%
  filter(`Service type - Care home service without nursing` == "Y")
CQC_care_homes_older_people_only <- CQC_care_homes_older_people %>% filter(is.na(`Service user band - Younger Adults`))
CQC_care_homes_older_people_only_nh <- CQC_care_homes_older_people_only %>%
  filter(`Service type - Care home service with nursing` == "Y") %>%
  filter(is.na(`Service type - Care home service without nursing`))
CQC_care_homes_older_people_only_res <- CQC_care_homes_older_people_only %>%
  filter(is.na(`Service type - Care home service with nursing`)) %>%
  filter(`Service type - Care home service without nursing` == "Y")
CQC_care_homes_older_people_only_nh_res <- CQC_care_homes_older_people_only %>%
  filter(`Service type - Care home service with nursing` == "Y") %>%
  filter(`Service type - Care home service without nursing` == "Y")
...
admissions_df <- data.frame(
  weekstart = c("15/06/2020", "30/03/2020"),
...
com_prev <- data.frame(
  ...
  median_sum_p = c(
    c(0.8855021569, 0.0002357542, 0.0000537222, 0.000120, 0.0002999070, 0.1137834217),
...
c_inf_t1 <- inf_states_t1 %>%
  filter(variable == "Ip" | variable == "Is" | variable == "Ia") %>%
  dplyr::summarise(median_sum_p = sum(median_sum_p))
v_inf_t1 <- inf_states_t1 %>%
  filter(variable == "Ip" | variable == "Ia") %>%
  dplyr::summarise(median_sum_p = sum(median_sum_p))
...
hospital_df <- data.frame(
  date = c("2020-07-15", "2020-05-01", "2020-04-01"),
...
p_S_vs_R_df <- data.frame(
  date = c("2020-07-15", "2020-05-01", "2020-04-01"),
  median_value = c(0.9408466, 0.9539467, 0.980676)
)
p_S_vs_R <- p_S_vs_R_df %>%
  filter(date == date_origin_hosp_prev) %>%
  select(median_value)
