...
percentiles_model <- function(N_param_set, N_sims_per_param_set) {
  ...
  ### variables
  state_names <- c(
    "S", "E", "Ipcpi", "Ipc", "Ia", "Iapi", "Ipci", "Iai",
  ...
  model.sim <- simulate(
    times = seq(from = 1, to = 100, by = timestep),
    t0 = 1,
  ...
  model.sim <- model.sim %>%
    group_by(.id) %>%
    mutate(cumsum_new_InfC_r = cumsum(new_InfC_r))
  ...
  df_p_InfC_r <- model.sim %>%
    group_by(time) %>%
    summarise(
      n.sims = sum(cumsum_new_InfC_r >= 0),
