...
outputs <- outputs %>%
  ...
  select(c("time", "p.InfC_r_O", "p.InfC_r_LO", "param.set", "scenario"))
...
p_more_than_0_residents_Ic_30d <- outputs_selected_R0 %>%
  filter(time == 30) %>%
  select(c("time", "p.InfC_r_O", "param.set", "scenario", "LTCF_type", "R0", "cprev"))

## p_more_than_9_residents_Ic_90d
p_more_than_9_residents_Ic_90d <- outputs_selected_R0 %>%
  filter(time == 90) %>%
  select(c("time", "p.InfC_r_LO", "param.set", "scenario", "LTCF_type", "R0", "cprev"))
...
# community prevalence
if (R0_scenario != "all") {
  p_more_than_0_residents_Ic_30d_nh$cprev <- factor(p_more_than_0_residents_Ic_30d_nh$cprev, levels = c(
    "low community prevalence",
    "medium community prevalence (baseline)",
    "high community prevalence"
  ))
  p_more_than_9_residents_Ic_90d_nh$cprev <- factor(p_more_than_9_residents_Ic_90d_nh$cprev, levels = c(
    "low community prevalence",
    "medium community prevalence (baseline)",
    "high community prevalence"
  ))
  p_more_than_0_residents_Ic_30d_res$cprev <- factor(p_more_than_0_residents_Ic_30d_res$cprev, levels = c(
    "low community prevalence",
    "medium community prevalence (baseline)",
    "high community prevalence"
  ))
  p_more_than_9_residents_Ic_90d_res$cprev <- factor(p_more_than_9_residents_Ic_90d_res$cprev, levels = c(
    "low community prevalence",
    "medium community prevalence (baseline)",
    "high community prevalence"
  ))
...
p_more_than_0_residents_Ic_30d_nh_plot <- p_more_than_0_residents_Ic_30d_nh %>%
  group_by(LTCF_type, R0, cprev, interventions) %>%
  summarise(
    dif_p.InfC_r_O_median = quantile(dif_p.InfC_r_O, probs = 0.5),
...
p_more_than_0_residents_Ic_30d_res_plot <- p_more_than_0_residents_Ic_30d_res %>%
  group_by(LTCF_type, R0, cprev, interventions) %>%
  summarise(
    dif_p.InfC_r_O_median = quantile(dif_p.InfC_r_O, probs = 0.5),
    dif_p.InfC_r_O_UL75 = quantile(dif_p.InfC_r_O, probs = 0.75),
...
p_more_than_9_residents_Ic_90d_nh_plot <- p_more_than_9_residents_Ic_90d_nh %>%
  group_by(LTCF_type, R0, cprev, interventions) %>%
  summarise(
    dif_p.InfC_r_LO_median = quantile(dif_p.InfC_r_LO, probs = 0.5),
...
p_more_than_9_residents_Ic_90d_res_plot <- p_more_than_9_residents_Ic_90d_res %>%
  group_by(LTCF_type, R0, cprev, interventions) %>%
  summarise(
    dif_p.InfC_r_LO_median = quantile(dif_p.InfC_r_LO, probs = 0.5),
    dif_p.InfC_r_LO_UL95 = quantile(dif_p.InfC_r_LO, probs = 0.95),
...
# order
ordered_int_df <- p_more_than_0_residents_Ic_30d_nh_plot %>%
  filter(var_order == order_by) %>%
  arrange(dif_p.InfC_r_O_median)
...
ordered_int_df <- p_more_than_0_residents_Ic_30d_res_plot %>%
  filter(var_order == order_by) %>%
  arrange(dif_p.InfC_r_O_median)
ordered_int <- ordered_int_df$interventions
...
ordered_int_df <- p_more_than_9_residents_Ic_90d_nh_plot %>%
  filter(var_order == order_by) %>%
  arrange(dif_p.InfC_r_LO_median)
ordered_int <- ordered_int_df$interventions
p_more_than_9_residents_Ic_90d_nh_plot$interventions <- factor(p_more_than_9_residents_Ic_90d_nh_plot$interventions,
  levels = ordered_int
)
ordered_int_df <- p_more_than_9_residents_Ic_90d_res_plot %>%
  filter(var_order == order_by) %>%
  arrange(dif_p.InfC_r_LO_median)
ordered_int <- ordered_int_df$interventions
...
plot_more_than_0_residents_Ic_30d_nh <- ggplot(data = p_more_than_0_residents_Ic_30d_nh_plot, aes(x = interventions, y = dif_p.InfC_r_O_median, shape = test_type)) +
  geom_point(size = 3, col = plot_col) +
  theme_bw() +
