...
delta_mean <- if (scenario_LTCF == "nh" & scenario_deaths == "Y") {
  round(daily_deathrate_woCOVID_MayJune2020_nh, 3)
} else if (scenario_LTCF == "res" & scenario_deaths == "Y") {
  round(daily_deathrate_woCOVID_MayJune2020_res, 4)
} else if (scenario_deaths == "N") {
  0
}
...
n_V_mean <- if (scenario_v == "N") {
  0
} else if (scenario_v == "prep") {
  (0.1 * 1) + (0.4 * (2 / 7)) + (0.4 * (1 / 14))
} else if (scenario_v == "weekly") {
  1 / 7
}
...
R0_LTCF_a_mean <- if (scenario_beta == "baseline") {
  2
} else if (scenario_beta == "change_R0") {
  changed_R0_a
}
...
m_R0_LTCF_b_vs_a_mean <- if (scenario_beta == "baseline") {
  0.5
} else if (scenario_beta == "change_R0") {
  changed_mR0_b
}
...
# R0 in the community
R0_community_mean <- if (scenario_h_imp == "only") {
  0
} else if (scenario_h_imp != "only") {
  changed_R0_community_mean
}
...
p_working_another_LTCF_mean <- if (scenario_h_imp == "only") {
  0
} else if (scenario_h_imp != "only" & scenario_staff_other_LTCF == "baseline") {
  0.01
} else if (scenario_h_imp != "only" & scenario_staff_other_LTCF == "change_p_working_another_LTCF") {
  changed_p_working_another_LTCF
}

# probability replacement staff work at another LTCF
p_working_another_LTCF2_mean <- if (scenario_h_imp == "only") {
  0
} else if (scenario_h_imp != "only") {
  0.2
}
...
m_beta2_mean <- if (scenario_beta2 == "baseline") {
  0.5
} else if (scenario_beta2 == "change_beta2") {
  changed_m_beta2
}
...
delay_to_isolation_nc_mean <- if (scenario_delay_isolation == "baseline") {
  2
} else if (scenario_delay_isolation == "change") {
  changed_delay_isolation
}

# delay from start of state to isolation
delay_to_isolation_c_mean <- if (scenario_delay_isolation == "baseline") {
  1
} else if (scenario_delay_isolation == "change") {
...
m_i_mean <- if (scenario_m_i == "baseline") {
  0.25
} else if (scenario_m_i == "change_m_i") {
  changed_m_i
}
...
p_tnc_mean <- if (scenario_p_tnc == "change_p_tnc") {
  changed_p_tnc
} else if (scenario_p_tnc == "baseline") {
  0.85
}

# probability of being tested if symptomatic with COVID-19
p_tc_mean <- if (scenario_p_tc == "change_p_tc") {
  changed_p_tc
} else if (scenario_p_tc == "baseline") {
  0.9
}

# probability of STAFF being tested if not symptomatic with COVID-19
p_tncs_mean <- if (scenario_p_tncs == "change_p_tncs") {
  changed_p_tncs
} else if (scenario_p_tncs == "baseline") {
  0.95
}

# probability of having a false-negative test in care home
p_fn_mean <- if (scenario_p_fn == "baseline") {
  0.2
} else if (scenario_p_fn == "change_p_fn") {
  changed_p_fn
}

# probability of having a false-negative test in hospital
p_fn_h_mean <- if (scenario_p_fn == "baseline") {
  0.1
} else if (scenario_p_fn == "change_p_fn") {
  changed_p_fn_h
}
...
p_i_mean <- if (scenario_isolation == "Y" & scenario_p_i == "baseline") {
  0.8
} else if (scenario_isolation == "Y" & scenario_p_i == "change_p_i") {
  changed_p_i
} else if (scenario_isolation == "N") {
  0
}

# case fatality ratio in residents
CFR_mean <- if (scenario_deaths == "Y") {
  0.25
} else if (scenario_deaths == "N") {
  0
}
...
# deaths in res vs nh
m_delta_c_res_mean <- if (scenario_LTCF == "nh") {
  1
} else if (scenario_LTCF == "res") {
  (((1 / 544.5) * 0.5) / ((1 / 283) * 0.55))
}
...
p_EIpcIa_hosp_mean <- if (scenario_h_inf_imp == "Y") {
  as.numeric(as.character(E_hospital_df_t1 + Ipc_hospital_df_t1 + Ia_hospital_df_t1))
} else if (scenario_h_inf_imp == "N") {
  0
}

# proportion still shedding after COVID-19 hospitalisation
p_shedding_after_H_mean <- if (scenario_COVID_returns == "all_I") {
  0.06
} else if (scenario_COVID_returns == "baseline" | scenario_COVID_returns == "all_R") {
  0
}
...
switch_h_imp <- if (scenario_h_imp == "N") {
  0
} else if (scenario_h_imp != "N") {
  1
}

# switch on new staff coming in
switch_new_staff <- if (scenario_replacement_staff == "Y") {
  1
} else if (scenario_replacement_staff == "N") {
  0
}
...
tnc_period <- if (scenario_f_tnc == "change_f_tnc") {
  changed_f_tnc
} else if (scenario_f_tnc == "baseline") {
  28
}

# periodic testing clinical residents
tc_period <- if (scenario_f_tc == "change_f_tc") {
  changed_f_tc
} else if (scenario_f_tc == "baseline") {
  0
}

# periodic testing non-clinical staff
tncs_period <- if (scenario_f_tncs == "change_f_tncs") {
  changed_f_tncs
} else if (scenario_f_tncs == "baseline") {
  7
}
...
switch_h_testing <- if (scenario_tested_after_H == "N") {
  0
} else if (scenario_tested_after_H == "Y") {
  1
}

# switches for all isolated after COVID-19 hospitalisation if still shedding vs not
switch_all_COVID_returns <- if (scenario_COVID_returns == "all_R") {
  2
} else if (scenario_COVID_returns == "all_I") {
  1
} else if (scenario_COVID_returns == "baseline") {
  0
}
